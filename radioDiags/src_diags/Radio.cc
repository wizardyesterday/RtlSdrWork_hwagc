...
/*****************************************************************************
  ...
  Purpose: The purpose of this function is to provide a mechanism
  for the AGC function to the gain of the current IF amplifier gain.
...
bool Radio::setReceiveIfGainInDb(uint8_t stage,uint32_t gain)
{
  ...
  registerValue  = analogAgcTable[gain];

  if (devicePtr != 0)
  {
    //-----------------------------------------------------
    // I'm testing a new concept for setting the IF gain
    // of the R82xx tuner by using the analog control line
    // from the RTL2832U devixe. Both the minum and maximum
    // analog voltate levels are set equal to each other.
    // Let's see if it bustikates the RTL2832U AGC.
    //-----------------------------------------------------
    // Minimum IF gain value.
    error = rtlsdr_demod_write_reg((rtlsdr_dev_t *)devicePtr,
                                   1,0x08,registerValue,1);

    // Maximum IF gain value.
    error = rtlsdr_demod_write_reg((rtlsdr_dev_t *)devicePtr,
                                   1,0x09,registerValue,1);
    //-----------------------------------------------------
...
/**************************************************************************
  ...
  Calling Sequence: gain = getReceive
...
/**************************************************************************
  ...
  Inputs:
    ...
    n_ring - An index that derives the divider value for the
    ring oscillator VCO.
